...
export default function SchedulePage() {
    ...
    const handleExportICal = async () => {
        try {
            ...
            const events = mapToCalendarEvents();
            ...
            link.download = `schedule_${semester}_${year}.ics`;
    ...
    const generateICalContent = (events) => {
        const lines = [
            'BEGIN:VCALENDAR',
            'VERSION:2.0',
            'PRODID:-//Smart Campus//Schedule//EN',
            'CALSCALE:GREGORIAN',
            'METHOD:PUBLISH',
        ...
        events.forEach(event => {
            ...
            lines.push(`UID:${event.id}@smartcampus.edu`);
            lines.push(`DTSTART:${formatICalDate(startDate)}`);
            lines.push(`DTEND:${formatICalDate(endDate)}`);
            lines.push(`SUMMARY:${event.title}`);
            lines.push(`LOCATION:${event.extendedProps.classroom}`);
            lines.push(`DESCRIPTION:${event.extendedProps.courseName || ''} - ${event.extendedProps.instructor || ''}`);
